...
// --- Utility Functions & Data (Retained) ---
...
// --- Helper Components (Simplified for brevity, inheriting theme via props) ---
...
function SalesPage() {
    ...
    // Handler for sub-menu items, closes mobile sidebar after click
    const handleSubMenuClick = (option) => {
        setActiveSubMenu(option);
        setIsMobileSidebarOpen(false); // Close sidebar on mobile after selecting an item
    };
    ...
    const renderMainContent = () => {
        ...
        return (
            <div className={`p-4 sm:p-8 rounded-xl ${contentBg} min-h-[700px]`}>
                ...
                {/* Content rendering logic (retained) */}
        ...
        <div className={`min-h-screen flex flex-col transition-colors duration-500 ${appBg}`}>
            ...
            <nav className={`sticky top-0 flex justify-between items-center px-4 sm:px-6 py-4 shadow-md z-20 ${navbarBg}`}>
                ...
                <div className="flex space-x-2 relative items-center">
                    ...
                    >
                        {/* SVG is omitted here for brevity, assume the sun/moon logic is still active */}
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            {isDarkMode ? (
                    ...
                    {/* Placeholder for User Dropdown/Admin (omitted for brevity) */}
                </div>
            </nav>

            {/* 2. Page Body (Responsive Layout) */}
